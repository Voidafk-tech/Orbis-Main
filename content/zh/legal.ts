...
export const PRIVACY: Widen<typeof En.PRIVACY> = {
  ...
  sections: [
    ...
    },
    {
      ...
      h: '网站分析',
      p: '我们使用 Google Analytics 了解访客浏览了哪些页面、其中哪些页面带来了查询，以便判断网站上哪些内容值得保留。它会记录浏览过的页面、是否提交了查询表格，以及是否使用了电话、邮件或微信联系方式，还有浏览器一般会发送的信息，例如大致位置、设备类型和来源网站。表格提交时，它还会记录你在下拉菜单中选择的四项答案——每月交易量大约多少、目前使用哪种软件、账目更新到什么程度，以及所属行业——以便我们了解网站实际带来的是哪一类查询。你亲手填写的内容不会被发送：包括姓名、企业名称、邮箱、电话，以及你用自己的话写下的任何内容。这些信息会发送给 Google，存放在他们位于加拿大境外的服务器上。它不会被用于识别你的个人身份。如果你不希望被统计，任何浏览器层面的追踪拦截工具都可以阻止它。',
    },
    {
      h: '数据保护',
